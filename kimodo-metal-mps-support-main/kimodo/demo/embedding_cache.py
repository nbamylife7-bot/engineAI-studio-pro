...
class CachedTextEncoder:
    ...
    def __init__(self, encoder, *, model_name: str, base_dir: Optional[str] = None):
        ...
        self.model_name = model_name
        q = getattr(encoder, "quantization", "")
        encoder_id = f"{type(encoder).__name__}_{q}" if q else f"{type(encoder).__name__}"
    ...
    def to(self, device=None, dtype=None):
        ...
        mode = getattr(self.encoder, "quantization", None)
        if mode and quantization_uses_bitsandbytes(mode):
            return self
