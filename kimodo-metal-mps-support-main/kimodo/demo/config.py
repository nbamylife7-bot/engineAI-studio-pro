...
# EngineAI T800 retargeting (GMR from web-version/gmr)
KIMODO_T800_ENABLED = os.environ.get("KIMODO_T800", "1").strip().lower() not in ("0", "false", "no")
DEFAULT_T800_SKIN = os.environ.get("KIMODO_T800_SKIN", "white").strip().lower()
# Optional: hide SMPL-X mesh when T800 robot is shown (default off — keep human + robot).
KIMODO_T800_HIDE_HUMAN_MESH = os.environ.get("KIMODO_T800_HIDE_HUMAN_MESH", "0").strip().lower() not in (
    "0",
    "false",
    "no",
)
