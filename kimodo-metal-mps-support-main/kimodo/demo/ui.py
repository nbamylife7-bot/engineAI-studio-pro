...
from kimodo.device_utils import tensor_from_numpy_on_device
...
from .scene_templates import (
    TEMPLATE_LABELS,
    get_scene_template_plan,
    list_scene_template_ids,
    prompt_frame_bounds,
)
...
def create_gui(
    ...
    from kimodo.demo.config import DEFAULT_T800_SKIN, KIMODO_T800_ENABLED
    from kimodo.retarget import is_t800_available
    from kimodo.viz.t800_rig import T800_SKIN_MODE_TO_UI, T800_SKIN_UI_OPTIONS, resolve_t800_skin_mode

    _t800_ui_available = KIMODO_T800_ENABLED and is_t800_available()
    _initial_t800_skin_ui = T800_SKIN_MODE_TO_UI.get(resolve_t800_skin_mode(DEFAULT_T800_SKIN), "White")
    ...
    with tab_group.add_tab("Generate", viser.Icon.WALK):
        ...
        with client.gui.add_folder("Scene templates", expand_by_default=True):
            client.gui.add_markdown(
                "Scene presets fill the timeline and constraints. Click **Generate** after picking one."
            )

            def apply_scene_template(template_id: str) -> None:
                session = demo.client_sessions.get(client_id)
                if session is None:
                    return
                try:
                    plan = get_scene_template_plan(template_id, fps=session.model_fps)
                ...
                if not HF_MODE:
                    gui_num_samples_slider.value = plan.num_samples
                if plan.enable_postprocess and INIT_POSTPROCESSING:
                    gui_postprocess_checkbox.value = True
                if plan.root2d_constraint is not None:
                    gui_dense_path_checkbox.value = False
                    if "2D Root" in session.constraints:
                        session.constraints["2D Root"].set_dense_path(False)

                frame_bounds = prompt_frame_bounds(plan.prompt_durations_sec, session.model_fps)
                total_frames = frame_bounds[-1][1] + 1 if frame_bounds else 1
                duration_sec = total_frames / session.model_fps
                set_new_duration(client_id, duration_sec)
                update_duration_gui(duration_sec)
                client.timeline.set_zoom_settings(default_num_frames_zoom=int(math.ceil(1.1 * total_frames)))

                for i, (prompt_text, (start_frame, end_frame)) in enumerate(
                    zip(plan.prompt_texts, frame_bounds)
                ):
                    color = PROMPT_COLORS[i % len(PROMPT_COLORS)]
                    client.timeline.add_prompt(prompt_text, start_frame, end_frame, color=color)

                if plan.root2d_constraint is not None:
                    load_constraints(client, [plan.root2d_constraint])

                demo.set_frame(client_id, 0)
                client.add_notification(
                    title=f"Template: {plan.title}",
                    body=plan.description,
                    auto_close_seconds=8.0,
                    color="blue",
                )

            for template_id in list_scene_template_ids():
                label = TEMPLATE_LABELS.get(template_id, template_id)
                btn = client.gui.add_button(label, icon=viser.Icon.WALK)

                @btn.on_click
                def _on_template(_event: viser.GuiEvent, tid: str = template_id) -> None:
                    apply_scene_template(tid)
        ...
        with client.gui.add_folder(
            ...
            def load_motion(client, load_path):
                ...
                if num_joints_loaded != num_joints_skeleton:
                    ...
                    if (
                        ...
                        if "local_rot_mats" in data:
                            local_rot_30 = tensor_from_numpy_on_device(data["local_rot_mats"], demo.device)
                ...
                try:
                    demo.retarget_t800_motions(client, session)
            ...
            def load_constraints(client, load_path_or_data):
                session = demo.client_sessions[client.client_id]
                constraints_lst = load_constraints_lst(load_path_or_data, skeleton=session.skeleton)
...
"""
        ...
        def apply_model_selection(new_model_name: str) -> None:
            ...
            if "smplx" not in new_model_name.lower() or not KIMODO_T800_ENABLED:
                demo._clear_t800_bootstrap(session)
                demo.clear_t800_motions(client_id)
            ...
            apply_constraint_overlay_visibility(session)
            if "smplx" in new_model_name.lower() and KIMODO_T800_ENABLED:
                session.t800_bootstrap_robot = None
                session.t800_preload_done.clear()
                session.t800_preload_in_progress = False
                demo.preload_t800_assets(client, session, force=True)
        ...
        def _update_visibility_for_loaded_model(loaded_model_name: str) -> None:
            ...
            _is_smplx = "smplx" in loaded_model_name.lower()
            gui_real_robot_rotations_checkbox.visible = _is_g1
            gui_t800_robot_checkbox.visible = _t800_ui_available and _is_smplx
            gui_t800_preload_markdown.visible = _t800_ui_available and _is_smplx
            gui_t800_skin_dropdown.visible = _t800_ui_available and _is_smplx and gui_t800_robot_checkbox.value
            gui_t800_quality_button.visible = _t800_ui_available and _is_smplx
            gui_t800_quality_markdown.visible = _t800_ui_available and _is_smplx
            gui_t800_export_pkl_button.visible = _t800_ui_available and _is_smplx
        ...
        def _(event: viser.GuiEvent) -> None:
            ...
            try:
                ...
                if _t800_ui_available and "smplx" in session.model_name.lower():
                    try:
                        demo.retarget_t800_motions(event.client, session)
                    except Exception as exc:
                        print(f"T800 retargeting failed: {exc}")
            except Exception as e:
                print(e)
    ...
    with tab_group.add_tab("Visualize", viser.Icon.EYE):
        ...
        with client.gui.add_folder("Body options", expand_by_default=True):
            ...
            gui_viz_skinned_mesh_opacity_slider = client.gui.add_slider(
                "Mesh Opacity", min=0.0, max=1.0, step=0.01, initial_value=0.55
            )
            ...
            gui_t800_robot_checkbox = client.gui.add_checkbox(
                "Show T800 robot (retargeted)",
                initial_value=_t800_ui_available,
                visible=_t800_ui_available and "smplx" in (model_name or "").lower(),
                hint="After Generate, retarget SMPL-X motion to EngineAI T800 (GMR) and show the robot mesh.",
            )
            gui_t800_preload_markdown = client.gui.add_markdown(
                "**T800:** loading gray robot mesh…",
                visible=_t800_ui_available and "smplx" in (model_name or "").lower(),
            )
            gui_t800_skin_dropdown = client.gui.add_dropdown(
                "T800 skin",
                options=T800_SKIN_UI_OPTIONS,
                initial_value=_initial_t800_skin_ui,
                visible=_t800_ui_available
                and "smplx" in (model_name or "").lower()
                and gui_t800_robot_checkbox.value,
                hint="White (default) loads quickly without textures. Textured uses MuJoCo materials.",
            )
            client.gui.add_markdown(
                "**T800 `.pkl`:** joint trajectory for sim/RL (Isaac Lab, etc.), not a direct real-robot command. "
                "Use **Download T800 PKL** below after Generate.",
                visible=_t800_ui_available and "smplx" in (model_name or "").lower(),
            )
            gui_t800_export_pkl_button = client.gui.add_button(
                "Download T800 PKL",
                visible=_t800_ui_available and "smplx" in (model_name or "").lower(),
                hint="Download retargeted T800 joint trajectory (.pkl) for the current character.",
            )
            gui_t800_quality_button = client.gui.add_button(
                "Quality check",
                visible=_t800_ui_available and "smplx" in (model_name or "").lower(),
                hint="Kimodo-calibrated audit: jumps, IK oscillation, velocity, limits, floor.",
            )
            gui_t800_quality_markdown = client.gui.add_markdown(
                "**T800 Quality:** click **Quality check** after Generate (Kimodo-calibrated scoring).",
                visible=_t800_ui_available and "smplx" in (model_name or "").lower(),
            )
        ...
        @gui_t800_robot_checkbox.on_update
        def _(_) -> None:
            if not demo.client_active(client_id):
                return
            session = demo.client_sessions[client_id]
            demo.set_t800_visibility(session, gui_t800_robot_checkbox.value)
            _is_smplx = "smplx" in session.model_name.lower()
            gui_t800_skin_dropdown.visible = (
                _t800_ui_available
                and _is_smplx
                and gui_t800_robot_checkbox.value
            )
        ...
        @gui_t800_quality_button.on_click
        def _(_event: viser.GuiEvent) -> None:
            if not demo.client_active(client_id):
                return
            session = demo.client_sessions[client_id]
            if not session.t800_motions:
                session.gui_elements.gui_t800_quality_markdown.content = (
                    "**T800 Quality:** no retarget yet — Generate with T800 enabled first."
                )
                return
            demo.run_t800_quality_check(session, notify=True)

        @gui_t800_export_pkl_button.on_click
        def _on_export_t800_pkl(event: viser.GuiEvent) -> None:
            if not demo.client_active(client_id):
                return
            event_client = event.client
            session = demo.client_sessions[client_id]
            if not session.t800_motions:
                event_client.add_notification(
                    title="No T800 motion",
                    body="Generate motion with T800 enabled first.",
                    auto_close_seconds=6.0,
                    color="red",
                )
                return
            try:
                from kimodo.retarget.t800_quality import motion_pkl_bytes

                character_name = next(iter(session.t800_motions.keys()))
                t800_motion = session.t800_motions[character_name]
                slug = character_name.strip().replace(" ", "_") or "character"
                filename = f"{slug}_t800.pkl"
                payload = motion_pkl_bytes(t800_motion.qpos_frames, float(t800_motion.motion_fps))
                event_client.send_file_download(filename, payload, save_immediately=True)
                event_client.add_notification(
                    title="T800 PKL download started",
                    body=filename,
                    auto_close_seconds=5.0,
                    color="green",
                )
                ...
                traceback.print_exc()
                event_client.add_notification(
                    title="Failed to export T800 PKL",
                    body=str(exc),
                    auto_close_seconds=10.0,
                    color="red",
                )

        @gui_t800_skin_dropdown.on_update
        def _(_) -> None:
            if not demo.client_active(client_id):
                return
            session = demo.client_sessions[client_id]
            demo.set_t800_skin(session, gui_t800_skin_dropdown.value)
            if not session.t800_motions and _t800_ui_available and "smplx" in session.model_name.lower():
                session.t800_bootstrap_robot = None
                session.t800_preload_done.clear()
                session.t800_preload_in_progress = False
                demo.preload_t800_assets(session.client, session, force=True)
    ...
    gui_elements = GuiElements(
        ...
        gui_t800_robot_checkbox=gui_t800_robot_checkbox,
        gui_t800_skin_dropdown=gui_t800_skin_dropdown,
        gui_t800_preload_markdown=gui_t800_preload_markdown,
        gui_t800_quality_button=gui_t800_quality_button,
        gui_t800_quality_markdown=gui_t800_quality_markdown,
        gui_t800_export_pkl_button=gui_t800_export_pkl_button,
    )
    return (
        gui_elements,
        timeline_tracks,
