...
def _extract_gradio_filepath(value) -> str:
    """Parse Gradio 4–6 filepath outputs (dict, FileData, or plain path)."""
    ...
    if path:
        return str(path)
    raise ValueError(f"Could not extract filepath from Gradio output: {value!r}")
...
class TextEncoderAPI:
    ...
    def __call__(self, texts):
        ...
        for text in texts:
            ...
            path = _extract_gradio_filepath(result[0])
            tensor = np.load(path)
