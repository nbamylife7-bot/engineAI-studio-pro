...
from kimodo.device_utils import resolve_text_encoder_device, resolve_torch_device
from kimodo.model.text_encoder_quantization import quantization_uses_bitsandbytes
...
def _mps_floating_dtype(dtype: torch.dtype) -> torch.dtype:
    """Return an MPS-compatible floating dtype."""
    ...
    for child in module.modules():
        for name, buffer in list(child._buffers.items()):
            if isinstance(buffer, torch.Tensor) and buffer.is_floating_point():
                target_dtype = _mps_floating_dtype(buffer.dtype)
                if target_dtype != buffer.dtype:
                    child._buffers[name] = buffer.to(dtype=target_dtype)
        for name, parameter in list(child._parameters.items()):
            if isinstance(parameter, nn.Parameter) and parameter.is_floating_point():
                target_dtype = _mps_floating_dtype(parameter.dtype)
                if target_dtype != parameter.dtype:
                    child._parameters[name] = nn.Parameter(
                        parameter.to(dtype=target_dtype),
...
def _move_module_to_mps(module: nn.Module, device: Union[str, torch.device]) -> None:
    ...
    def _convert(tensor: torch.Tensor) -> torch.Tensor:
        ...
        if tensor.is_floating_point():
            target_dtype = _mps_floating_dtype(tensor.dtype)
...
def _text_encoder_quantization_mode(text_encoder) -> Optional[str]:
    inner = getattr(text_encoder, "encoder", text_encoder)
    return getattr(inner, "quantization", None)


def _bnb_text_encoder_loaded(text_encoder) -> bool:
    mode = _text_encoder_quantization_mode(text_encoder)
    return bool(mode and quantization_uses_bitsandbytes(mode))


def _move_text_encoder_to_device(text_encoder, device: Union[str, torch.device]) -> None:
    """Move encoder wrappers whose inner HF model is an nn.Module."""
    if _bnb_text_encoder_loaded(text_encoder):
        # matbee NF4: weights stay on GPU via device_map; do not .to(cpu/mps).
        return

    if isinstance(text_encoder, nn.Module):
        if str(device).lower().startswith("mps"):
            _normalize_mps_floating_tensors(text_encoder)
            _move_module_to_mps(text_encoder, device)
    ...
    if isinstance(inner, nn.Module):
        if str(device).lower().startswith("mps"):
            _normalize_mps_floating_tensors(inner)
            _move_module_to_mps(inner, device)
        else:
            inner.to(device)
        return
...
class Kimodo(nn.Module):
    ...
    def __init__(
        ...
        device = resolve_torch_device(str(device) if device is not None else None)
        self.device = device
        text_encoder_device = resolve_text_encoder_device(device)
        self.text_encoder_device = text_encoder_device

        if str(device).lower().startswith("mps"):
            for module in (self.denoiser, self.diffusion, self.sampler):
                _normalize_mps_floating_tensors(module)
                _move_module_to_mps(module, device)
        elif device is not None:
            self.denoiser.to(device)
            self.diffusion.to(device)
            self.sampler.to(device)

        _move_text_encoder_to_device(self.text_encoder, text_encoder_device)
