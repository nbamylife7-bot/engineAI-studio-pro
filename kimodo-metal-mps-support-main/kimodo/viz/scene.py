...
class Character:
    def __init__(
        ...
        if create_skinned_mesh:
            if isinstance(self.skeleton, (SOMASkeleton30, SOMASkeleton77)) and mesh_mode in [
                ...
                min_height = bind_pos[:, 1].min().item()
                bind_pos[:, 1] -= min_height
                skin_device = getattr(self.skin, "compute_device", bind_pos.device)
                bind_pos = bind_pos.to(skin_device)
                bind_rotmat = torch.eye(3, device=skin_device).repeat(bind_pos.shape[0], 1, 1)
                self.set_pose(bind_pos, bind_rotmat)
                self.skinned_mesh.visible = True
                self.set_skinned_mesh_visibility(visible_skinned_mesh)
                self.set_skinned_mesh_opacity(skinned_mesh_opacity)
    ...
    def precompute_skinning(self, joints_pos: torch.Tensor, joints_rot: torch.Tensor):
        ...
        from kimodo.device_utils import release_device_memory

        assert self.skin is not None
        num_frames = joints_pos.shape[0]
        skin_device = getattr(self.skin, "compute_device", joints_pos.device)

        lazy_threshold = int(os.environ.get("KIMODO_LAZY_SKIN_FRAMES", "0"))
        if lazy_threshold > 0 and num_frames > lazy_threshold:
            self.skinned_verts_cache = None
            return

        chunk_size = max(1, int(os.environ.get("KIMODO_SKIN_CHUNK_SIZE", "48")))
        vert_count = int(self.skin.bind_vertices.shape[0])
        cache = np.zeros((num_frames, vert_count, 3), dtype=np.float32)

        for start in range(0, num_frames, chunk_size):
            end = min(start + chunk_size, num_frames)
            chunk_pos = joints_pos[start:end].to(skin_device)
            chunk_rot = joints_rot[start:end].to(skin_device)
            chunk_verts = self.skin.skin(chunk_rot, chunk_pos, rot_is_global=True).detach().cpu().numpy()
            cache[start:end] = chunk_verts
            del chunk_pos, chunk_rot, chunk_verts
            release_device_memory(skin_device)

        self.skinned_verts_cache = cache
    ...
    def set_pose(
        ...
        if self.skinned_mesh is not None:
            if self.skinned_verts_cache is not None:
                ...
                skin_device = getattr(self.skin, "compute_device", joints_rot.device)
                skinned_verts = self.skin.skin(
                    joints_rot[None].to(skin_device),
                    joints_pos[None].to(skin_device),
                    rot_is_global=True,
                )[0].cpu().numpy()
    ...
    def clear(self):
        if self.skeleton_mesh is not None:
            ...
            if hasattr(self.skeleton_mesh, "mesh_info_cache"):
                self.skeleton_mesh.mesh_info_cache = None
        if self.skinned_mesh is not None:
