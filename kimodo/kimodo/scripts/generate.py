...
from kimodo.device_utils import resolve_torch_device, tensor_from_numpy_on_device
...
def resolve_generation_device(args: argparse.Namespace, meta: Optional[Dict[str, Any]] = None) -> str:
    """Resolve the torch device from CLI flags and optional meta.json overrides."""
    device_arg = args.device
    if device_arg == "auto" and meta and isinstance(meta.get("device"), str):
        device_arg = meta["device"]
    return resolve_torch_device(device_arg)
...
def main():
    ...
    device = resolve_generation_device(args, meta)
    ...
    if args.bvh:
        ...
        if "somaskel" not in skeleton.name:
            ...
            if n_samples == 1:
                ...
                joints_pos = tensor_from_numpy_on_device(output["posed_joints"][0], device)
                joints_rot = tensor_from_numpy_on_device(output["global_rot_mats"][0], device)
                ...
                for i in range(n_samples):
                    joints_pos = tensor_from_numpy_on_device(output["posed_joints"][i], device)
                    joints_rot = tensor_from_numpy_on_device(output["global_rot_mats"][i], device)
                    local_rot_mats = global_rots_to_local_rots(joints_rot, skeleton)
                    root_positions = joints_pos[:, skeleton.root_idx, :]
                    save_motion_bvh(
