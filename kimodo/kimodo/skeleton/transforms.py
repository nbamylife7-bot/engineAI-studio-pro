...
def global_rots_to_local_rots(global_joint_rots: torch.Tensor, skeleton):
    ...
    """
    device, dtype = global_joint_rots.device, global_joint_rots.dtype
    joint_parents = skeleton.joint_parents.to(device=device)
    ...
    parent_rot_mats = global_joint_mats[:, joint_parents]

    parent_rot_mats[:, skeleton.root_idx] = torch.eye(3, device=device, dtype=dtype)  # the root joint
