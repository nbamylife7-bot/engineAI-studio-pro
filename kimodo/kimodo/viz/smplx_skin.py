...
class SMPLXSkin:
    def __init__(
        ...
        from kimodo.device_utils import resolve_skin_compute_device

        device = resolve_skin_compute_device(skeleton.neutral_joints.device)
    ...
    def skin(self, joint_rotmat, joint_pos, rot_is_global=False):
        ...
        joint_rotmat = joint_rotmat.to(device=compute_device)
        joint_pos = joint_pos.to(device=compute_device)
        if joint_rotmat.is_floating_point():
            joint_rotmat = joint_rotmat.float()
        if joint_pos.is_floating_point():
            joint_pos = joint_pos.float()
        ...
        if rot_is_global:
            if joint_rotmat.shape[1] == self.full_joint_count:
                ...
                parent_rot_mats[:, self.full_root_idx] = torch.eye(3, device=device, dtype=joint_rotmat.dtype)
        ...
        if rot_is_global and joint_rotmat.shape[1] == self.full_joint_count:
            ...
            full_local = torch.eye(3, device=device, dtype=joint_rotmat.dtype).reshape(1, 1, 3, 3).repeat(
                nF, self.full_joint_count, 1, 1
            )
        ...
        pose_feature = (full_local[:, 1:] - torch.eye(3, device=device, dtype=joint_rotmat.dtype)[None, None]).reshape(
            nF, -1
        )
        ...
        neutral_root = self.skeleton.neutral_joints[0:1].to(device=device, dtype=joint_pos.dtype)
        root_trans = joint_pos[:, self.skeleton.root_idx] - neutral_root
        posed_joints = posed_joints + root_trans[:, None, :]

        fk_transform = torch.eye(4, device=device, dtype=joint_rotmat.dtype)[None, None].repeat(
            nF, self.full_joint_count, 1, 1
        )
